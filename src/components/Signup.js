...
const Signup = () => {
	...
	const register = async () => {
		console.log(name);
		try {
			const res = await axios.post("http://localhost:8081/api/v1/register/user", {
				name: name,
				email: username,
				password: password,
			});
			...
			<section className="flex flex-col md:flex-row justify-center space-y-10 md:space-y-0 md:space-x-16 items-center my-2 mx-5 md:mx-0 md:my-0">
				<div className="md:w-1/3 max-w-sm">
					...
					<div className="mt-4 font-semibold text-sm text-slate-500 text-center md:text-left">
						...
						<a className="text-black hover:underline hover:underline-offset-4" href="/login">
							Login
						</a>
					</div>
				</div>
